...
def RXEQ(y, t, I, VMAX, KM, CUE, fPHYS, fCHEM, fAVAI, FI, tau, LITmin, SOMmin, MICtrn, desorb, DEsorb, OXIDAT, KO):
    ...
    LIT_1 = y[0] if y[0] > 0 else 1e-6
    LIT_2 = y[1] if y[1] > 0 else 1e-6
    MIC_1 = y[2] if y[2] > 0 else 1e-6
    MIC_2 = y[3] if y[3] > 0 else 1e-6
    SOM_1 = y[4] if y[4] > 0 else 1e-6
    SOM_2 = y[5] if y[5] > 0 else 1e-6
    SOM_3 = y[6] if y[6] > 0 else 1e-6
    ...
    MICtrn[0] = MIC_1 * tau[0]  * fPHYS[0]
    MICtrn[1] = MIC_1 * tau[0]  * fCHEM[0]
    MICtrn[2] = MIC_1 * tau[0]  * fAVAI[0]
    ...
    MICtrn[3] = MIC_2 * tau[1]  * fPHYS[1]
    MICtrn[4] = MIC_2 * tau[1]  * fCHEM[1]
    MICtrn[5] = MIC_2 * tau[1]  * fAVAI[1]
    ...
    OXIDAT    = ((MIC_1 * VMAX[1] * SOM_2 / (KO[0]*KM[1] + SOM_2)) +
               (MIC_2 * VMAX[4] * SOM_2 / (KO[1]*KM[4] + SOM_2)) )  #oxidation of C to A

    dLIT_1 = I[0]*(1-FI[0]) - LITmin[0] - LITmin[2]

    dMIC_1 = CUE[0]*(LITmin[0]+ SOMmin[0]) + CUE[1]*(LITmin[1]) - (MICtrn[0] + MICtrn[1] + MICtrn[2])
    dSOM_1 = I[0]*FI[0] + MICtrn[0] + MICtrn[3] - DEsorb

    dLIT_2 = I[1] * (1-FI[1]) - LITmin[1] - LITmin[3]
    dMIC_2 = CUE[2]*(LITmin[2]+ SOMmin[1]) + CUE[3]*(LITmin[3]) - (MICtrn[3] + MICtrn[4] + MICtrn[5])
    dSOM_2 = I[1]*FI[1] + MICtrn[1] + MICtrn[4] - OXIDAT

    dSOM_3 = MICtrn[2] + MICtrn[5] + DEsorb + OXIDAT - SOMmin[0] - SOMmin[1]

    return np.array([dLIT_1, dLIT_2, dMIC_1, dMIC_2, dSOM_1, dSOM_2, dSOM_3])